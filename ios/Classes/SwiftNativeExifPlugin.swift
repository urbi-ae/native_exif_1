...
private class NativeExifInterface {
  ...
  func getMetadata() throws -> NSDictionary {
    ...
    guard let metadata = CGImageSourceCopyPropertiesAtIndex(source, 0, nil) else {
      throw NativeExifError(
        code: "READ_ERROR", message: "Error while reading metadata from source", details: nil)
    }
...
public class SwiftNativeExifPlugin: NSObject, FlutterPlugin {
  ...
  private func getArguments(_ callArguments: Any?) throws -> [String: Any] {
    guard let arguments = callArguments as? [String: Any] else {
  ...
  private func getExifData(_ arguments: [String: Any]) throws -> NSMutableDictionary {
    ...
    guard let interface = interfaces[id] else {
      throw NativeExifError(
        code: "NOT_FOUND", message: "No ExifInterface was found with given id", details: nil)
    }
    ...
    if metadata[kCGImagePropertyExifDictionary as String] != nil {
      exif.addEntries(
        from: metadata[kCGImagePropertyExifDictionary as String] as! [AnyHashable: Any])
    ...
    if metadata[kCGImagePropertyTIFFDictionary as String] != nil {
      exif.addEntries(
        from: metadata[kCGImagePropertyTIFFDictionary as String] as! [AnyHashable: Any])
    }
  ...
  public func setAttributes(
    id: Int, attributes: [String: AnyObject], result: @escaping FlutterResult
  ) throws {
    guard let interface = interfaces[id] else {
      throw NativeExifError(
        code: "NOT_FOUND", message: "No ExifInterface was found with given id", details: nil)
    ...
    guard let uniformTypeIdentifier = CGImageSourceGetType(source) else {
      throw NativeExifError(
        code: "SRC_ERROR", message: "Error while getting source type", details: nil)
    }

    guard let destination = CGImageDestinationCreateWithURL(url, uniformTypeIdentifier, 1, nil)
    else {
      throw NativeExifError(
        code: "WRITE_ERROR", message: "Error while creating destination for writing metadata",
        details: nil)
    }

    guard var metadata = try interface.getMetadata() as? [String: AnyObject] else {
      throw NativeExifError(
        code: "READ_ERROR", message: "Metadata could not be converted to mutable dictionary",
        details: nil)
    }
    ...
    var exif =
      ((metadata[kCGImagePropertyExifDictionary as String] as? NSDictionary)?.mutableCopy()
      ?? [String: AnyObject]()) as! [String: AnyObject]
    var tiff =
      ((metadata[kCGImagePropertyTIFFDictionary as String] as? NSDictionary)?.mutableCopy()
      ?? [String: AnyObject]()) as! [String: AnyObject]
    var gps =
      ((metadata[kCGImagePropertyGPSDictionary as String] as? NSDictionary)?.mutableCopy()
      ?? [String: AnyObject]()) as! [String: AnyObject]

    for value in attributes {
      if value.key == "Orientation" {
        metadata[kCGImagePropertyOrientation as String] = value.value as AnyObject
      } else if value.key == "Artist" {
        tiff[kCGImagePropertyTIFFArtist as String] = value.value as AnyObject
      } else if value.key == "Software" {
        tiff[kCGImagePropertyTIFFSoftware as String] = value.value as AnyObject
      } else if value.key == "SubSecTime" {
        ...
        if let stringValue = value.value as? String, let yResolution = Double(stringValue) {
          exif[kCGImagePropertyExifFocalPlaneYResolution as String] = NSNumber(value: yResolution)
        }
      } else if value.key == "FocalPlaneXResolution" {
        if let stringValue = value.value as? String, let xResolution = Double(stringValue) {
          exif[kCGImagePropertyExifFocalPlaneXResolution as String] = NSNumber(value: xResolution)
        }
    ...
    guard CGImageDestinationFinalize(destination) else {
      throw NativeExifError(
        code: "WRITE_ERROR", message: "Error while finalizing destination for writing metadata",
        details: nil)
    }
  }
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "getAttribute":
      do {
        ...
        guard let tag = arguments["tag"] as? String else {
          result(
            FlutterError(
              code: "BAD_ARGUMENTS", message: "tag field must be of type string.", details: nil))
          return
        }
    ...
    case "setAttribute":
      do {
        let arguments = try getArguments(call.arguments)
        guard let id = arguments["id"] as? Int else {
          throw NativeExifError.badArguments(
            message: "id field must be of type integer.", details: nil)
        }

        guard let tag = arguments["tag"] as? String else {
          throw NativeExifError.badArguments(
            message: "tag field must be of type string.", details: nil)
        }
    ...
    case "setAttributes":
      do {
        ...
        guard let attributes = arguments["values"] as? [String: AnyObject] else {
          throw NativeExifError.badArguments(
            message: "values field must be of type Dictionary.", details: nil)
        }
    ...
    case "close":
      do {
        ...
        guard let id = arguments["id"] as? Int else {
          result(
            FlutterError(
              code: "BAD_ARGUMENTS", message: "id field must be of type integer.", details: nil))
          return
        }
    ...
    default:
      result(
        FlutterError(
          code: "NOT_IMPLEMENTED", message: "The given method is not implemented!", details: nil))
    }
  }
}
